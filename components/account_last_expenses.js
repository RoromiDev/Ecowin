...
export class AccountLastExpenses extends Component {
    ...
    sortByDate() {
        ...
        for (let i = 0; i < this.state.transactions.length; i++) {
            if (this.state.transactions[i].account == this.props.account.name && this.state.transactions[i].amount < 0) {
        ...
        return (
            <View style={{ marginTop: 0 }}>
                ...
                {this.sortByDate().length != 0 ?
                    <TouchableOpacity onPress={() => { this.props.navigation.navigate('AllExpensesList', { accounts: this.props.account, active: this.props.active }) }}>
                        <CardItem style={{ borderBottomWidth: 1, borderBottomColor: 'rgb(248, 248, 248)' }}>
                            <Text style={{ flex: 3, color: 'rgb(200, 200, 200)', fontFamily: 'Avenir', fontWeight: '300', fontSize: 16 }}>Voir toutes les dépenses</Text>
                            <Right style={{ flex: 1 }}>
